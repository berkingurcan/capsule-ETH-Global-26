...
import { ROLES, type Role } from "@/lib/capsule/roles";
import { PARENT_NAME_DISPLAY } from "@/lib/capsule/public-env";
import {
  PROVIDERS,
  PROVIDER_IDS,
  SUGGESTED_MODELS,
...
/* ------------------------------------------------------------------
   ...
   Which labels are already taken is read from the chain by the server
   component that renders this one — it used to be a hardcoded `taken`
   flag on the analyst preset, which was wrong the moment anything else
   was minted.

   Nothing here signs anything yet; the mint is the next step.
   ------------------------------------------------------------------ */
...
export default function LaunchFlow({ taken }: { taken: string[] }) {
  ...
  const [picked, setPicked] = useState<string[]>(free);
  ...
  function toggle(r: Role) {
    if (taken.includes(r.slug)) return;
    setPicked((p) => {
...
function StepParent({ next }: { next: () => void }) {
  ...
  const checks: [string, string, boolean][] = [
    ...
    ["You own " + PARENT_NAME_DISPLAY, "checked at mint, against ETHRegistry", false],
          ...
          <div className="hint" style={{ fontSize: 12.5 }}>
            {custom ? (
              <span style={{ color: "var(--mint-700)", fontWeight: 600 }}>
                {custom}.{PARENT_NAME_DISPLAY} is free
              </span>
            ) : (
...
function StepMint({ drafts, back, next }: { drafts: Draft[]; back: () => void; next: () => void }) {
  ...
  const lines = useMemo(
    () => [
      ...
      "register subname under " + PARENT_NAME_DISPLAY,
    ...
    <div className="panel pad-lg">
      ...
      <div className="grid g-side">
        <div className="tile">
          ...
          <div className="stack">
            {[
              ["Register", drafts.map((d) => d.slug).join(", ") + " under " + PARENT_NAME_DISPLAY],
            ...
            <pre className="term" style={{ fontSize: 11.5, minHeight: 150 }}>
              {BOOT.slice(0, n).map((l, k) => (
                <span key={k} className={k === BOOT.length - 1 ? "g" : "d"}>
                  {l.replace("NAME", d.slug + "." + PARENT_NAME_DISPLAY)}
                  {"\n"}
                </span>
              ))}
