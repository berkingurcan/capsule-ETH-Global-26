...
/* ------------------------------------------------------------------
   The launchpad. Five steps, one form — the order matters: credentials
   are entered before the mint so the runner has everything it needs the
   moment the name exists, and the record only ever holds a pointer.
   Nothing here signs anything; it is a design demo.

   There is no payment step. x402 was cut on 2026-09-08 along with
   ERC-4337 — see DECISIONS.md in the branding repo. Minting is free on
   the hackathon deployment, and a fee, if there is ever one, is a
   msg.value check on CapsuleMinter.mint() rather than a sixth screen.
   ------------------------------------------------------------------ */
...
type Draft = {
  ...
  key: string;
  /**
   * Numeric Telegram user ids allowed to DM this agent. Required, not optional: a bot
   * anyone can DM is a prompt-injection surface reachable from a search box, and ENS
   * cannot defend against it — the injected instruction never touches a record.
   */
  allowFrom: string;
};
  ...
  return (
    <main className="page">
      <div className="wrap wrap-narrow">
        ...
        {step === 3 && <StepMint drafts={drafts} next={() => setStep(4)} />}
...
function StepConfigure({
  ...
  return (
    <div className="panel pad-lg">
      ...
      <p className="ssub">
        Everything on the left is written into the ENS record and read by the runner at boot — the keys are the ones
        ENSIP-26 and ENSIP-27 define, so any client that speaks them can read this agent without knowing what Capsule
        is. Everything on the right is a secret: it goes to the encrypted store, and the record holds only a pointer.
      </p>
      ...
      <div className="grid g-side">
        <div className="col" style={{ gap: 18 }}>
          ...
          <div className="field">
            ...
            <input
              id="context"
              className="input"
              value={d.title + " for " + PARENT.name}
              readOnly
            />
            <span className="hint">
              ENSIP-26. What a generic ENS client shows someone who resolves this name.
            </span>
          </div>
        </div>
        ...
        <div className="col" style={{ gap: 18 }}>
          ...
          <div className="field">
            ...
            <span className="hint">
              From @BotFather. Yours, not ours — it is sealed in the store and handed to the OpenClaw gateway at boot.
            </span>
          </div>
          ...
          <div className="field">
            ...
            <input
              ...
              className="input"
              value={d.allowFrom}
              onChange={(e) => patch(i, { allowFrom: e.target.value.replace(/[^0-9, ]/g, "") })}
            />
            <span className="hint">
              Numeric Telegram user ids. Required — an agent anyone can message is one anyone can try to talk out of
              its instructions, and that attack never touches a record ENS could protect.
            </span>
          </div>
          ...
          <div className="tile shell">
            ...
            <pre className="term" style={{ fontSize: 11.5, lineHeight: 1.7 }}>
              <span className="d">class </span>
              <span className="w">Agent</span>
              {"\n"}
              <span className="d">schema </span>
              <span className="w">/schema/capsule-agent-v1.json</span>
              {"\n"}
              ...
              {"\n"}
              <span className="d">agent-endpoint[web] </span>
              <span className="w">t.me/{d.slug}_bot</span>
              {"\n"}
...
function StepMint({ drafts, next }: { drafts: Draft[]; next: () => void }) {
  ...
  const lines = useMemo(
    () => [
      ...
      "setText ×9 · class, schema, agent-context, endpoints, model, runtime, prompt",
      "setText agent-registration[0x0001…a362][7]   ENSIP-25",
      "authorizeTextRoles(name, agent-heartbeat, agent, true)",
      "emit CapsuleMinted",
  ...
  return (
    <div className="panel pad-lg">
      ...
      <p className="ssub">
        Without the minter this is a dozen separate calls per agent. With it, the subname, the records and the
        heartbeat role land together — or none of them do. Minting is free on the hackathon deployment.
      </p>
        ...
        <div className="tile">
          ...
          <div className="stack">
            {[
              ...
              ["Write records", "ENSIP-26 and ENSIP-27 keys, read by the runner at boot"],
              ["Prove the registration", "ENSIP-25 — the name verifiably belongs to this agent"],
              ["Grant the role", "agent-heartbeat — the only thing the agent may write"],
              ["Emit CapsuleMinted", "so the subgraph sees it in one event"],
            ].map(([a, b]) => (
...
const BOOT = [
  ...
  "addr claims this runner · booting",
  "prompt cap_8f3d1a unsealed",
  "openclaw gateway started · telegram online",
...
function StepLive({ drafts }: { drafts: Draft[] }) {
  ...
  return (
    <div className="panel pad-lg">
      ...
      <p className="ssub">
        Each agent gets the same container, and the only thing that distinguishes one from another is{" "}
        <span className="mono">CAPSULE_NAME</span>. It resolves itself, proves the name claims its key, fetches its
        prompt and its bot token over a signed request, and starts an OpenClaw gateway on your Telegram bot.
      </p>
      ...
      {done && (
        <div className="notice mint" style={{ marginTop: 22 }}>
          ...
          <p style={{ margin: 0 }}>
            ...
            <span className="mono">agent-prompt</span> in the record and the supervisor rewrites the gateway&rsquo;s
            workspace and restarts it within 30 seconds: same container, different agent, no redeploy.
          </p>
        </div>
      )}
